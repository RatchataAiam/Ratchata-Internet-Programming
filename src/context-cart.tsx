...
export function CartProvider({ children }: { children: React.ReactNode }) {
  ...
  const addItem = (item: Omit<CartItem, "key">) => {
    ...
    setItems((current) => {
      ...
      if (!existing) {
        return [
          ...current,
          {
            ...item,
            key,
            quantity: item.item_type === "Cat" ? 1 : Math.max(1, item.quantity),
          },
        ];
      }
      ...
      const max = Math.max(
        0,
        Number(item.stock_quantity ?? existing.stock_quantity ?? 999999),
      );
      const next = Math.min(
        existing.quantity + Math.max(1, item.quantity),
        max || existing.quantity + Math.max(1, item.quantity),
      );
      return current.map((x) =>
        x.key === key
          ? {
              ...x,
              quantity: next,
              stock_quantity: item.stock_quantity ?? x.stock_quantity,
            }
          : x,
      );
    });
      ...
      current.map((item) => {
        ...
        const max =
          item.item_type === "Product"
            ? Number(item.stock_quantity ?? 999999)
            : 1;
