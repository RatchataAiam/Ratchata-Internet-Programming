...
const TIER_META: Record<PriceTier, { fg: string; bg: string; icon: keyof typeof Ionicons.glyphMap; hint: string }> = {
  High: { fg: '#B93859', bg: 'rgba(251, 113, 133, 0.14)', icon: 'trending-up', hint: 'Premium-priced items' },
  Medium: { fg: '#B45309', bg: 'rgba(251, 191, 36, 0.14)', icon: 'remove', hint: 'Mid-range priced items' },
  Low: { fg: '#3E8F68', bg: 'rgba(52, 211, 153, 0.14)', icon: 'trending-down', hint: 'Budget-friendly items' },
};
...
export default function PriceClustersScreen() {
  ...
  const loadClusters = async () => {
    try {
      ...
      setError(err instanceof Error ? err.message : 'ไม่สามารถเชื่อมต่อ Server ได้');
    } finally {
      setLoading(false);
    }
  };
    ...
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.header}>
        ...
        <View style={{ flex: 1 }}>
          ...
          <Text style={styles.headerSubtitle}>Automatic High / Medium / Low grouping</Text>
        </View>
        ...
        <ScrollView contentContainerStyle={styles.scrollContent}>
          <View style={styles.infoCard}>
            ...
            <Text style={styles.infoText}>
              Prices are grouped into tiers with an unsupervised k-means model, run fresh on every
              refresh so new products are re-clustered automatically — no manual price bands needed.
            </Text>
          </View>
          ...
          <View style={styles.summaryRow}>
            {TIER_ORDER.map((tier) => {
              ...
              return (
                <View key={tier} style={[styles.summaryCard, { borderColor: meta.fg }]}>
                  <View style={[styles.summaryIconWrap, { backgroundColor: meta.bg }]}>
                    <Ionicons name={meta.icon} size={16} color={meta.fg} />
                  </View>
                  <Text style={[styles.summaryTier, { color: meta.fg }]}>{tier}</Text>
                  <Text style={styles.summaryCount}>{count} items</Text>
                  {typeof centroid === 'number' && (
                    <Text style={styles.summaryAvg}>avg ฿{centroid.toLocaleString()}</Text>
                  )}
                </View>
              );
            })}
              ...
              <View key={tier} style={styles.section}>
                ...
                >
                  <View style={styles.sectionHeaderLeft}>
                    <View style={[styles.tierDot, { backgroundColor: meta.fg }]} />
                ...
                {expanded && (
                  <View style={styles.sectionBody}>
                    {items.length === 0 ? (
                      <Text style={styles.emptyTierText}>ไม่มีสินค้าในกลุ่มนี้</Text>
                    ) : (
                      items.map((item) => (
                        <TouchableOpacity
                          key={item.product_id}
                        ...
                        >
                          ...
                          <Text style={[styles.productPrice, { color: meta.fg }]}>
...
const styles = StyleSheet.create({
  ...
  summaryCount: { fontSize: 11, color: COLORS.textSecondary, fontWeight: '600' },
  ...
  emptyTierText: { color: COLORS.textMuted, fontSize: 12, padding: 10, textAlign: 'center' },
  productRow: {
    flexDirection: 'row',
    alignItems: 'center',
    gap: 10,
  ...
  productImage: { width: 40, height: 40, borderRadius: 8, backgroundColor: COLORS.surfaceElevated },
  ...
  retryBtn: { backgroundColor: COLORS.primary, paddingHorizontal: 22, paddingVertical: 10, borderRadius: 8 },
