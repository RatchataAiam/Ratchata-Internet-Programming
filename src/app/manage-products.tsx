...
export default function ManageProducts() {
  ...
  // Load the Products table for the admin list and dashboard data.
  const load = async () => {
    setLoading(true);
    try {
      const r = await fetch(API);
      if (!r.ok) throw new Error("โหลดสินค้าไม่สำเร็จ");
  ...
  // The same handler supports both INSERT and UPDATE operations.
  const save = async () => {
    if (
      ...
      notify("ข้อมูลไม่ครบ", "กรุณากรอกชื่อ หมวดหมู่ และราคา");
      return;
    }
    const body = {
      ...form,
    ...
    try {
      const r = await fetch(editing ? `${API}/${editing.product_id}` : API, {
      ...
      setForm(empty);
      await load();
      notify("สำเร็จ", editing ? "แก้ไขสินค้าแล้ว" : "เพิ่มสินค้าแล้ว");
    } catch (e) {
  ...
  const filteredItems = useMemo(() => {
    ...
    return items.filter(
      (p) =>
        (categoryFilter === "All" ||
          (p.category || "ไม่ระบุหมวดหมู่") === categoryFilter) &&
        (!q ||
          `${p.product_id} ${p.product_name} ${p.category || ""} ${p.description || ""}`
            .toLowerCase()
            .includes(q)),
    );
  ...
  const categories = useMemo(
    () => [
      "All",
      ...Array.from(new Set(items.map((p) => p.category || "ไม่ระบุหมวดหมู่"))),
    ],
    [items],
  );
  ...
  const del = (p: Product) => {
    const run = async () => {
      try {
        const r = await fetch(`${API}/${p.product_id}`, {
          method: "DELETE",
    ...
    if (
      Platform.OS === "web" ? window.confirm(`ลบ ${p.product_name} ?`) : true
    ) {
      if (Platform.OS === "web") run();
      else
        Alert.alert("ยืนยันการลบ", p.product_name, [
          { text: "ยกเลิก" },
  ...
  return (
    <SafeAreaView style={s.container}>
      <View style={s.header}>
        ...
        <View>
          <Text style={s.title}>จัดการสินค้า</Text>
          <Text style={s.sub}>Products ตามโครงสร้าง SQL</Text>
        </View>
      ...
      <ScrollView contentContainerStyle={s.content}>
        <View style={s.form}>
          ...
          {[
            ["product_name", "ชื่อสินค้า", "Royal Canin Persian Adult"],
            ["category", "หมวดหมู่", "Cat Food"],
            ["price", "ราคา", "850"],
            ["stock_quantity", "จำนวนสต็อก", "20"],
        ...
        <Text style={s.section}>
          รายการสินค้า ({filteredItems.length}/{items.length})
        </Text>
        <View style={s.search}>
          ...
          <TextInput
            ...
            placeholder="ค้นหาชื่อสินค้า / Category / ID"
            placeholderTextColor="#B49BA4"
            style={s.searchInput}
          />
          {search !== "" && (
        ...
        >
          {categories.map((cat) => (
            <TouchableOpacity
              key={cat}
              style={[s.filterBtn, categoryFilter === cat && s.filterBtnActive]}
              onPress={() => setCategoryFilter(cat)}
            >
              <Text
                style={[
                  s.filterText,
                  categoryFilter === cat && s.filterTextActive,
                ]}
              >
                {cat === "All" ? "ทั้งหมด" : cat}
              </Text>
          ...
          categories
            ...
            .map((cat) => (
              <View key={cat}>
                  ...
                  <Text style={s.groupCount}>
                    {
                      filteredItems.filter(
                        (p) => (p.category || "ไม่ระบุหมวดหมู่") === cat,
                      ).length
                    }{" "}
                    รายการ
                  </Text>
                ...
                {filteredItems
                  .filter((p) => (p.category || "ไม่ระบุหมวดหมู่") === cat)
                  .map((p) => (
                    <View key={p.product_id} style={s.card}>
                      {p.image_url ? (
                        <Image source={{ uri: p.image_url }} style={s.image} />
                      ) : (
                      ...
                      <View style={s.actions}>
                        <TouchableOpacity onPress={() => edit(p)}>
                          <Ionicons
                            name="create-outline"
                            size={21}
                            color={C.pink}
...
const s = StyleSheet.create({
  ...
  price: { fontSize: 15, fontWeight: "900", color: C.pinkDark, marginTop: 4 },
