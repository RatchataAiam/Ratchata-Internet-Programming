...
export default function ManageBreeds() {
  ...
  // โหลดข้อมูลทุกแถวจาก Breeds ก่อนแสดงฟอร์มและรายการสำหรับผู้ดูแลระบบ
  const load = async () => {
    setLoading(true);
    try {
      const r = await fetch(API);
      if (!r.ok) throw new Error("โหลดสายพันธุ์ไม่สำเร็จ");
  ...
  // ฟอร์มนี้เชื่อมตรงกับ breed_name, description และ image_url
  const save = async () => {
    if (!form.breed_name.trim()) {
      notify("ข้อมูลไม่ครบ", "กรุณากรอกชื่อสายพันธุ์");
      return;
    }
    const body = {
    ...
    try {
      const r = await fetch(editing ? `${API}/${editing.breed_id}` : API, {
  ...
  const del = (b: Breed) => {
    const run = async () => {
      try {
        const r = await fetch(`${API}/${b.breed_id}`, {
          method: "DELETE",
    ...
    if (Platform.OS === "web") {
      if (window.confirm(`ลบ ${b.breed_name} ?`)) run();
    } else {
      Alert.alert("ยืนยันการลบ", b.breed_name, [
        { text: "ยกเลิก" },
  ...
  const filteredItems = useMemo(() => {
    ...
    return items.filter(
      (b) =>
        !q ||
        `${b.breed_id} ${b.breed_name} ${b.description || ""}`
          .toLowerCase()
          .includes(q),
    );
  }, [items, search]);
  return (
    <SafeAreaView style={s.container}>
      <View style={s.header}>
        ...
        <View>
          <Text style={s.title}>จัดการสายพันธุ์</Text>
          <Text style={s.sub}>Breeds ตามโครงสร้าง SQL</Text>
        </View>
      ...
      <ScrollView contentContainerStyle={s.content}>
        <View style={s.form}>
          <Text style={s.section}>
            {editing ? "แก้ไขสายพันธุ์" : "เพิ่มสายพันธุ์"}
          </Text>
          <Text style={s.label}>ชื่อสายพันธุ์</Text>
          <TextInput
            style={s.input}
            value={form.breed_name}
            onChangeText={(v) => setForm({ ...form, breed_name: v })}
            placeholder="Himalayan Cat"
          ...
          <View style={s.formBtns}>
            ...
            <TouchableOpacity style={s.save} onPress={save}>
              <Text style={s.saveText}>
                {editing ? "บันทึกการแก้ไข" : "เพิ่มสายพันธุ์"}
              </Text>
            </TouchableOpacity>
          </View>
        </View>
        <Text style={s.section}>
          รายการสายพันธุ์ ({filteredItems.length}/{items.length})
        </Text>
        <View style={s.search}>
          ...
          <TextInput
            ...
            placeholder="ค้นหาชื่อ Breed / Breed ID"
            placeholderTextColor="#B49BA4"
            style={s.searchInput}
          />
          {search !== "" && (
        ...
        {loading ? (
          ...
          <View style={s.empty}>
            ...
            <Text style={s.muted}>ไม่พบสายพันธุ์ที่ค้นหา</Text>
          </View>
        ) : (
          filteredItems.map((b) => (
            <View key={b.breed_id} style={s.card}>
              {b.image_url ? (
                <Image source={{ uri: b.image_url }} style={s.image} />
              ) : (
              ...
              <View style={{ flex: 1 }}>
                ...
                <Text style={s.meta} numberOfLines={2}>
                  {b.description || "ไม่มีรายละเอียด"}
                </Text>
              </View>
              <View style={s.actions}>
                <TouchableOpacity onPress={() => edit(b)}>
                  <Ionicons name="create-outline" size={21} color={C.pink} />
                </TouchableOpacity>
                <TouchableOpacity onPress={() => del(b)}>
